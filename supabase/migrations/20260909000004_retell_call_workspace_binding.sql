...
DECLARE
  candidate uuid;
BEGIN
  IF NEW.workspace_id IS NOT NULL OR NEW.provider <> 'retell' THEN
    RETURN NEW;
  END IF;
  ...
  WITH candidates AS (
    ...
    JOIN workspace_contacts AS wc
      ON wc.workspace_id = state.workspace_id
     AND wc.contact_id = NEW.contact_id
     AND wc.lifecycle_status = 'active'
    WHERE state.conversation_id = NEW.conversation_id
      AND state.contact_id = NEW.contact_id
    UNION
    SELECT state.workspace_id
  ...
  SELECT (array_agg(workspace_id ORDER BY workspace_id))[1] INTO candidate
  FROM candidates
  HAVING count(DISTINCT workspace_id) = 1;
...
BEGIN
  IF OLD.source_turn_id IS DISTINCT FROM NEW.source_turn_id
     ...
     OR (OLD.workspace_id IS NULL AND NEW.workspace_id IS NOT NULL AND NEW.workspace_id IS DISTINCT FROM (
       ...
       SELECT (array_agg(workspace_id ORDER BY workspace_id))[1]
       FROM candidates
       HAVING count(DISTINCT workspace_id) = 1
     ))
