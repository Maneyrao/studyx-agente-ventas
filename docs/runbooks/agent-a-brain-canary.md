...
3. Deben estar presentes, sin imprimir valores: la key HMAC del orquestador, el
   signing secret, `GROQ_API_KEY`, credenciales PostgreSQL y credenciales de canal.
4. Gates locales: 20/20 conversaciones held-out; naturalidad >=18/20; p95 del
   cerebro <=4,5 s; unitarias, integraciones seleccionadas, ambos typechecks y
...
| Canary | `true` | `false` | Brain V1 autoritativo con planner y egress backend. |
...
Los 429 se registran como fallos de transporte. El runner puede respetar
`Retry-After` de forma acotada, pero nunca los convierte en éxito conversacional.
...
3. Desplegar Botpress con el mismo SHA/configuración.
