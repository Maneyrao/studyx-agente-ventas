...
--
-- Plain column additions: safe to apply twice (IF NOT EXISTS / DROP+ADD
-- CONSTRAINT), no new function, no privilege change.
...
ALTER TABLE outbound_deliveries
  ...
  CHECK (deferred_lead_projection_applied_on IS NULL
         OR deferred_lead_projection_applied_on IN ('accepted'));
